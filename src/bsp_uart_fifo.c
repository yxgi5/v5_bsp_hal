...
*       并修改 UartVarInit()中的函数指针等?诒竞热? g_tUart2.SendBefor = RS485_SendBefor
* 形    参: 无
* 返 回 值: 无
*********************************************************************************************************
*/
...
static void InitHardUart(void)
{
  ...
  CLEAR_BIT(USART1->SR, USART_FLAG_TC); /* 清除TC发送完成标志 */
  CLEAR_BIT(USART1->SR, USART_FLAG_RXNE);  /* 清除RXNE接收标志 */
  // USART_CR1_PEIE | USART_CR1_RXNEIE /* 使能PE. RX接受中断 */
  ...
  CLEAR_BIT(USART2->SR, USART_FLAG_TC); /* 清除TC发送完成标志 */
  CLEAR_BIT(USART2->SR, USART_FLAG_RXNE);/* 清除RXNE接收标志 */
  ...
  CLEAR_BIT(USART3->SR, USART_FLAG_TC); /* 清除TC发送完成标志 */
  CLEAR_BIT(USART3->SR, USART_FLAG_RXNE);/* 清除RXNE接收标志 */
  ...
  CLEAR_BIT(USART6->SR, USART_FLAG_TC); /* 清除TC发送完成标志 */
  CLEAR_BIT(USART6->SR, USART_FLAG_RXNE);/* 清除RXNE接收标志 */
...
static void UartIRQ(UART_T *_pUart)
{
  uint32_t srflags   = READ_REG(_pUart->uart->SR);
  uint32_t cr1its     = READ_REG(_pUart->uart->CR1);
  uint32_t cr3its     = READ_REG(_pUart->uart->CR3);

  /* 处理接收中断  */
  if ((srflags & USART_FLAG_RXNE) != RESET)
  {
  ...
  /* 处理发送缓冲区空中断 */
  if ( ((srflags & USART_FLAG_TXE) != RESET) && (cr1its & USART_CR1_TXEIE) != RESET)
  {
  ...
  /* 数据bit位全部发送完毕的中断 */
  if (((srflags & USART_FLAG_TC) != RESET) && ((cr1its & USART_CR1_TCIE) != RESET))
  {
    //if (_pUart->usTxRead == _pUart->usTxWrite)
    if (_pUart->usTxCount == 0)
    {
